...
void onClientTalkState(client_t** clients, uint16_t userId, uint8_t state) {
    int idx = findClient(clients, userId);
    if(idx != -1) {
        uint8_t oldState = clients[idx]->state;
        if(state) { clients[idx]->state |= B0001; }
        else { clients[idx]->state &= B1110; }

        int line = findLine(clients, idx);
        drawUserState(line, oldState, clients[idx]->state);
    }
}
...
void onClientMicState(client_t** clients, uint16_t userId, uint8_t state) {
    ...
    if(idx != -1) {
        uint8_t oldState = clients[idx]->state;
        if(state) { clients[idx]->state |= B0010; }
        else { clients[idx]->state &= B1101; }
...
void onClientSoundState(client_t** clients, uint16_t userId, uint8_t state) {
    ...
    if(idx != -1) {
        uint8_t oldState = clients[idx]->state;
        if(state) { clients[idx]->state |= B0100; }
        else { clients[idx]->state &= B1011; }
...
void onClientAwayState(client_t** clients, uint16_t userId, uint8_t state) {
    ...
    if(idx != -1) {
        uint8_t oldState = clients[idx]->state;
        if(state) { clients[idx]->state |= B1000; }
        else { clients[idx]->state &= B0111; }
